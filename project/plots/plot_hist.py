...
if __name__ == "__main__":
    ...
    plt.plot(np.arange(float(S_sum.shape[0]))/10, S_sum, label="Susceptible")
    plt.plot(np.arange(float(S_sum.shape[0]))/10, I_sum, label="Infectious")
    plt.plot(np.arange(float(S_sum.shape[0]))/10, R_sum, label="Recovered")
    plt.legend()
    plt.xlabel("Time")
    plt.ylabel("People")
