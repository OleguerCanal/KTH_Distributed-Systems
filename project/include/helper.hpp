...
void receive_people(int from, std::vector<Person> *people) {
    ...
    for(int i = 0; i < size; i += 3) {
        Person person(encoded_people[0], encoded_people[1], (int) encoded_people[2]);
        people->push_back(person);
    }
}

void exchange_people(int p, int P,
                    const std::list<Person> &people_to_prev_region,
                    const std::list<Person> &people_to_next_region,
